      ...
      cell: (row) => (
        <>
          {editCountryId === row.countryId ? (
            <div className="p-2">
              <label className="--switch mt-1">
                <input
                  type="checkbox"
                  id="dashboardStats"
                  checked={selectedCountryData.all}
                  name="all"
            ...
            <Label variant="ghost" color={(row.all && 'success') || 'error'}>
              {row.all ? 'Yes' : 'No'}
            </Label>
          )}
        </>
...

    {
      ...
      cell: (row) => (
        <>
          {editCountryId !== row.countryId ? (
            <>
              <button className="btn btn-sm btn-primary" onClick={() => editData(row)}>
                <Icon icon={editfill} />
              </button>
              &nbsp;
            </>
            ...
            <>
              <button className="btn btn-sm btn-primary" onClick={() => updateCountrySalesTax()}>
                <Icon icon="fa-solid:save" />
              </button>
              &nbsp;
            </>
          )}
          ...
          <button
            ...
            onClick={(e) =>
              navigate('/admin/setting/sales-tax/' + row.countryId, {
                state: { countryName: row.countryDetails.country }
              })
            }
          >
            State
